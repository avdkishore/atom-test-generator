...
function addProps(code, parsedData, cb) {
  ...
  try {
    ...
    Object.keys(coponentInfo[0].props).forEach((key) => {
      parsedData.props.push([key, mockValue(coponentInfo[0].props[key])]);
    });
...
export default function parse(filePath, cb, codeInFile) {
  ...
  const moduleName = (path.extname(filePath) === '.js') && path.basename(filePath);

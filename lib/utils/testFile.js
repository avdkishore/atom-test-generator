...
export function getTestFileData({ exportedClass = '', entries = [] }) {
  ...
  return { fileData, entries, exportedClass, propObjString, wrapperString };
}

export function getRawFileData(data) {
  let entries = [];
  let exportedClass = '';
...
export default function testFile(src, cb = () => {}) {
  return fs.readFile(src, 'utf8', (err, data) => {
    ...
    if (data) {
      const { fileData, exportedClass, entries } = getTestFileData(getRawFileData(data));
      // create file path
